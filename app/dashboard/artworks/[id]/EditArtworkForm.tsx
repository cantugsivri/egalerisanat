...
export default function EditArtworkForm({ artwork }: EditArtworkFormProps) {
  ...
  // Parser function to split "80x100 cm" or "Çap: 50 cm"
  const parseDimensions = (dimStr: string) => {
    let w = '';
    let h = '';
    let d = '';
    
    if (!dimStr) return { w, h, d };
    
    // Check diameter: "Çap: 50 cm" or "Çap: 50" or "Ø 50"
    const diameterMatch = dimStr.match(/(?:Çap|Ø)\s*:\s*([\d.,]+)/i) || dimStr.match(/(?:Çap|Ø)\s*([\d.,]+)/i);
    if (diameterMatch) {
      d = diameterMatch[1].trim();
    }
    
    // Check width x height: "80x100" or "80 x 100 cm"
    const cleanedStr = dimStr.replace(/(?:Çap|Ø).+$/i, '').trim();
    const mainMatch = cleanedStr.match(/^([\d.,]+)\s*x\s*([\d.,]+)/i);
    if (mainMatch) {
      w = mainMatch[1].trim();
      h = mainMatch[2].trim().replace(/\s*cm$/i, '');
    } else {
      const wMatch = cleanedStr.match(/En\s*:\s*([\d.,]+)/i);
      const hMatch = cleanedStr.match(/Boy\s*:\s*([\d.,]+)/i);
      if (wMatch) w = wMatch[1].trim();
      if (hMatch) h = hMatch[1].trim();
      
      if (!wMatch && !hMatch && !diameterMatch) {
        w = dimStr.replace(/\s*cm$/i, '').trim();
      }
    }
  ...
  const [width, setWidth] = useState(parsed.w)
  const [height, setHeight] = useState(parsed.h)
  const [diameter, setDiameter] = useState(parsed.d)
  ...
  const [imageUrl, setImageUrl] = useState<string | null>(artwork.imageUrl)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || 'Eser güncellenemedi.')
      } else {
    ...
    <div style={{ maxWidth: 800, margin: '0 auto' }}>
      <div className="page-header">
        <div>
          ...
          <h1 className="page-title">Eseri Düzenle</h1>
          <p className="page-subtitle">Sanat eserinizin bilgilerini güncelleyin.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="card" style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
          ...
          >
            {submitting ? '' : 'Değişiklikleri Kaydet'}
          </button>
        </div>
